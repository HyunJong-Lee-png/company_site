...
export default async function Business({ searchParams }: { searchParams: { category?: string } }) {
  const cookieStore = await cookies();
  const language = (cookieStore.get('language')?.value || 'ko') as 'ko';
  const t = translations[language];
  const categoryKey = (searchParams.category || 'liner-agent') as 'liner-agent' | 'ship-agent' | 'logistics' | 'real-estate';
